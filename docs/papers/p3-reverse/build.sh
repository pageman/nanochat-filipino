...
DL_OUT="/Users/paulpajo/Downloads/nanochat-filipino/p3/paper_outputs"
mkdir -p "$OUT" "$DL_OUT"
...
if command -v tectonic >/dev/null 2>&1; then
  ...
  (cd "$OUT" && pdflatex -interaction=nonstopmode paper.tex >/tmp/p3_latex1.log \
    && pdflatex -interaction=nonstopmode paper.tex >/tmp/p3_latex2.log \
    && pdflatex -interaction=nonstopmode paper.tex >/tmp/p3_latex3.log)
else
...
pandoc --from latex --to html5 --standalone --mathjax --toc \
  --metadata title="Tagalog Retention and English Acquisition under Equal-Budget nanochat Continual Pretraining" \
  "$ROOT/paper.tex" -o /tmp/p3_paper_raw.html
python3 "$ROOT/inject_css.py" /tmp/p3_paper_raw.html "$OUT/paper.html"
