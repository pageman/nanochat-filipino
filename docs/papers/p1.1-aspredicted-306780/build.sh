...
pandoc --from latex --to html5 --standalone --mathjax --toc \
  --metadata title="Equal-Exposure Depth and Held-Out Tagalog Bits-per-Byte on WikiText-TL-39" \
  "$ROOT/paper.tex" -o /tmp/p11_paper_raw.html
python3 "$ROOT/inject_css.py" /tmp/p11_paper_raw.html "$OUT/paper.html"
