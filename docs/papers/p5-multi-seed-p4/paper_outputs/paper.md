...
Pajo [@pajo2026p11] measured equal-exposure Tagalog BPB across nanochat depths on WikiText-TL-39 (AsPredicted #306780). Pajo [@pajo2026p2] asked the forward continual-pretraining question from an English parent (AsPredicted #306935). Pajo [@pajo2026p3] asked the reverse from a fresh Tagalog parent, with a 50/50-*document* B3 arm (AsPredicted #307342). Pajo [@pajo2026p4] locked source-content token share at $q_{\mathrm{TL}}=0.50$ after a fresh Tagalog parent (AsPredicted #307591) and observed both co-primary inequalities in *one* initialization (P4 seed 0).

P5 was designed after those released P4 findings were known. It is therefore a prospectively preregistered **post-P4** multi-seed panel of the frozen P4 apparatus [@aspredicted307836]. It is not an amendment, correction, rescue, replication, or independent confirmation of P4. It is not "P4 confirmed as a law." C3 remains the P4-frozen token-share mix; C3 is not P3 B3. P4 seed 0 is historical disclosure, not a P5 confirmatory cell.
...
Cruz and Cheng constructed WikiText-TL-39 [@cruz2019eval]. Sequential interference is classical [@mccloskey1989catastrophic; @french1999cf]. LLM continual-pretraining work often continues an English or multilingual base [@luo2023forgetting; @ramasesh2021forgetscale; @shi2024continual]. P5 stays inside WikiText-family text, the nanochat depth dial, and BPB rather than CORE or chat [@karpathy2026nanochat]. Equal token budgets follow the family so stream identity is not confounded with compute [@kaplan2020scaling; @hoffmann2022chinchilla; @muennighoff2023dataconstrained]. WikiText established clean Wikipedia LM evaluation in English [@merity2017pointer]. BPB remains comparable across vocabularies [@shannon1951prediction; @brown2020gpt3]. Official P5 BPB is mean token NLL divided by $\ln 2$ times mean UTF-8 bytes per token, full split, $T=2048$, BOS-best-fit. In-loop trainer BPB is not confirmatory. Preregistration reduces undisclosed flexibility [@nosek2018prereg; @wagenmakers2012confirmatory]. A closed count of unused initializations is not a random-effects model and is not a population rate.

# Methods {#sec:methods}

## Registration and authority

AsPredicted #307836 governs [@aspredicted307836].
...
Tagalog train/val/test documents are the frozen WikiText-TL-39 P1.1-eligible splits. English train/val/test documents are the frozen WikiText-103 P2-eligible splits. The tokenizer is the carry-forward P4 pair: `tokenizer.pkl` and `token_bytes.pt` . P1.1/P2/P3/P4 weights were not loaded as parents. Official GPU gates used NVIDIA CUDA A40. Official evaluation used `scripts/p5/evaluate_bpb.py`.

## Panel seeds and parent initialization
...
P0-T compared full-split Tagalog validation BPB of d8 and d20 TL0 against a seed-matched untrained initialization and a byte-unigram floor at $\delta_{\mathrm{P0T}}=0.01$. Official P0-T status is CUDA-only; safe public output before Gate X was restricted to PASS, BLOCKED, or TECHNICAL_STOP. After PASS, d20 was copied to an immutable $C0_s$ directory with `additional_train_tokens`=0. Smoke and d8 checkpoints are not C0. A BLOCKED seed would have been reported as ineligible, not replaced; that branch was not triggered.

## Children and C3 identity

C1, C2, and C3 each continued the same frozen $C0_s$ for $N=294$ steps, $B=65{,}536$, $D_{\mathrm{phase2}}=19{,}267{,}584$ tokens, serial R$\to$S$\to$T, `load_optimizer=False`, child peak learning rate $0.3\times$ the parent peak, warmup 14, terminal checkpoint only. C1 used extra Tagalog. C2 used WikiText-103 raw English train. C3 used the P4-frozen mix (not reshuffled): source-content quotas $9{,}633{,}792$ / $9{,}633{,}792$, interleave seed 42, mix-manifest SHA-256 . Packed C3 shard hashes and `language_origin_mask` SHA-256 were verified; matching source JSONLs or the manifest alone is not sufficient C3 identity.

## Lockbox, analysis, and tests

Validation BPB was lockboxed until one panel Gate X. Each eligible seed's Gate U sealed six child cells plus descriptive C0 English with `test_access[`$s$`]`=0 and no contrasts computed at seal. Policy authorized one C3-only secondary test event after U (Gate V). C1 and C2 were not tested. Contrasts use sealed validation cells; tests are secondary and excluded from classification. Equality at $-\delta$ counts. No composite score, mean, confidence interval, $p$-value, or random-effects model.

# Results {#sec:results}

All three seeds completed the filed path through V. P0-T status is **PASS** for seeds 1, 2, and 3. Table [1](#tab:p0t){reference-type="ref" reference="tab:p0t"} reports Tagalog validation BPB of the trained parents against the two floors. Table [2](#tab:cells){reference-type="ref" reference="tab:cells"} reports the sealed child cells. Table [3](#tab:contrasts){reference-type="ref" reference="tab:contrasts"} reports per-seed contrasts and class. Table [4](#tab:panel){reference-type="ref" reference="tab:panel"} is the primary panel result.
  ...
  : P0-T Tagalog full-split validation BPB (six decimals). Unigram floor is $4.453225$ for all seeds. {#tab:p0t}
  ...
  : Full-split validation BPB after terminal d20 checkpoints. Six-decimal reporting as filed. C0 English is descriptive and excluded from the contrasts. {#tab:cells}
  ...
  : Per-seed co-primary contrasts from sealed validation cells. Filed criterion $\le -0.01$; equality counts. {#tab:contrasts}
  ...
  : Primary panel result: counts in the closed $K=3$ panel. This table *is* the panel claim. {#tab:panel}
...
A measured reduction in the P3-style relative Tagalog retention cost within this frozen P5 replica of the P4 apparatus, not a general mitigation of catastrophic forgetting. That optional sentence is per eligible seed (all three met both criteria) and does not replace Table [4](#tab:panel){reference-type="ref" reference="tab:panel"} as the primary panel claim. It does not appear in the title.
...
AsPredicted #307836. P5: multi-seed panel of the P4 token-share mix after fresh TL parents. <https://aspredicted.org/k6ib64.pdf>, 2026.
