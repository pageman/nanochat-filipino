#!/usr/bin/env bash
# P3 Gate T: B3 frozen 50/50 mix from frozen B0.
set -euo pipefail
...
LOCK="${P3_LOCKBOX_ROOT}/gate-t-b3-full.log"
mkdir -p "$(dirname "$LOCK")"
: >"${P3_SAFE_PROGRESS_ROOT}/gate-t-b3-progress.txt"
echo "gate_t_started utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"${P3_SAFE_PROGRESS_ROOT}/gate-t-b3-progress.txt"
set +e
...
python scripts/p3/gate_phase2_accept.py --gate T --arm b3 --model-tag "$TAG" \
  --stream b3_mix --data-dir "$NANOCHAT_DATA_DIR" --exit-code "$RC"
exit $?
