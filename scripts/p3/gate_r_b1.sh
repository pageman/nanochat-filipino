#!/usr/bin/env bash
# P3 Gate R: B1 extra-Tagalog from frozen B0.
set -euo pipefail
...
TAG="p3-b1-extra-tl-d20"
FROZEN="${NANOCHAT_BASE_DIR}/b0/frozen/p3-tl0-d20"
LOCK="${P3_LOCKBOX_ROOT}/gate-r-b1-full.log"
mkdir -p "$(dirname "$LOCK")"
: >"${P3_SAFE_PROGRESS_ROOT}/gate-r-b1-progress.txt"
echo "gate_r_started utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"${P3_SAFE_PROGRESS_ROOT}/gate-r-b1-progress.txt"
set +e
...
python scripts/p3/gate_phase2_accept.py --gate R --arm b1 --model-tag "$TAG" \
  --stream tagalog_train --data-dir data/processed/p3-tl39-active --exit-code "$RC"
exit $?
