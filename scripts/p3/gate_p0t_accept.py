#!/usr/bin/env python3
"""Gate P0-T accept: lockbox scalars -> safe PASS/BLOCKED only."""
...
DETAIL = BASE / "lockbox" / "gate-p0-t-eval-detail.json"
ELIG = BASE / "lockbox" / "gate-p0-t-eligibility.json"
SAFE = BASE / "safe_progress" / "gate-p0-t-status.json"
LOCK_LOG = BASE / "lockbox" / "gate-p0-t-eval-full.log"
P0_T_MARGIN = 0.01
...
def main() -> int:
    ...
    technical = args.eval_exit_code != 0 and not DETAIL.is_file()
    ...
    receipt = {
        ...
        "researchbox_id": RESEARCHBOX_ID,
        "gate": "P0-T",
        "status": "pass" if status == "PASS" else ("blocked" if status == "BLOCKED" else "technical_block"),
        "p0_t_status": status,
        "at_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "host": platform.node(),
        "p3_run_id": P3_RUN_ID,
        "script": "scripts/p3/gate_p0t.sh",
        "preflight_script": "scripts/p3/gate_p0t_preflight.py",
        "accept_script": "scripts/p3/gate_p0t_accept.py",
        "eval_script": "scripts/p3/evaluate_bpb.py",
        "margin_bpb": P0_T_MARGIN,
        "depths": depths_safe,
        "test_access": 0,
        "english_val_used": False,
        ...
        "safe_status": str(SAFE.relative_to(ROOT)),
