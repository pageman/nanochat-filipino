...
CKPT = BASE / "base_checkpoints" / "p4-smoke-tl-d4" / "model_000030.pt"
QUAR = LOCKBOX / "quarantine" / "p4-smoke-tl-d4"
C0 = BASE / "c0"
...
def quarantine(src: Path) -> dict:
    ...
    smoke_dir = BASE / "base_checkpoints" / "p4-smoke-tl-d4"
    moved = []
    if smoke_dir.is_dir():
    ...
    in_c0 = list(C0.rglob("*.pt")) if C0.is_dir() else []
    ...
    return {
        ...
        "in_c0": [str(p.relative_to(ROOT)) for p in in_c0],
    }
...
def main() -> int:
    ...
    payload = {
        ...
        "blinded": True,
        "p4_run_id": P4_RUN_ID,
        "script": "scripts/p4/gate_h_smoke.sh",
        "model_tag": "p4-smoke-tl-d4",
        "depth": 4,
        "num_iterations": 30,
        "warmup_steps": 3,
        "eval_every": -1,
    ...
    if health:
        ...
        lock["gate_statuses"]["H"] = "pass"
        lock["status"] = "gate_h_pass"
        lock["parent_status"] = "not_started"
        write_json(LOCK_PATH, lock)
        mark_ledger("H", "pass", str(OUT.relative_to(ROOT)), "I")
    blinded_print(
        "H",
        payload["status"],
        {
