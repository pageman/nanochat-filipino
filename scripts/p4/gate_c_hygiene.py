...
def main() -> int:
    ...
    record("C-03_no_prior_checkpoint_in_p4_cache", ckpts == [], ckpts[:5])
    ...
    record(
        ...
        test_in_train == [] and os.environ.get("P4_TEST_JSONL_EN") in (None, "") and os.environ.get("P4_TEST_JSONL_TL") in (None, ""),
        {"hits": test_in_train},
    )

    secret_hits = []
    for rel in git("ls-files").stdout.splitlines():
        if rel.startswith("scripts/p4/") or rel.startswith("docs/run-cards/p4/"):
            path = ROOT / rel
    ...
    env_text = (ROOT / "scripts/p4/env.sh").read_text(encoding="utf-8")
    record(
    ...
    record(
        ...
        os.environ.get("P3_ENV_SOURCED") is None and os.environ.get("P2_RUN_ID") is None and os.environ.get("P1_RUN_ID") is None,
        True,
    )
    ...
    payload = {
        ...
        "blinded": True,
        "p4_run_id": P4_RUN_ID,
        "script": "scripts/p4/gate_c_hygiene.py",
        "checks": checks,
        "test_access_count": 0,
