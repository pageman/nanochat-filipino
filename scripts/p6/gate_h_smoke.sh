#!/usr/bin/env bash
# P6 Gate H: Tagalog-path d4 CUDA smoke (NOT TL0 / NOT C0 / NOT per-seed parent).
set -euo pipefail
...
AUTH="$P6_RUN_CARD_ROOT/gate-h-authorization.json"
if [[ ! -f "$AUTH" ]]; then
...
import os
auth = Path(os.environ["P6_RUN_CARD_ROOT"]) / "gate-h-authorization.json"
...
LOCK_LOG="${P6_LOCKBOX_ROOT}/gate-h-smoke-full.log"
SAFE="${P6_SAFE_PROGRESS_ROOT}/gate-h-smoke-progress.txt"
mkdir -p "$(dirname "$LOCK_LOG")" "$P6_SAFE_PROGRESS_ROOT"
: >"$SAFE"
...
cd "$ROOT"
python scripts/p6/gate_h_accept.py --exit-code "$RC"
exit $?
