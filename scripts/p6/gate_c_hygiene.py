...
def main() -> int:
    ...
    record(
        ...
        test_in_train == [] and os.environ.get("P6_TEST_JSONL_EN") in (None, "") and os.environ.get("P6_TEST_JSONL_TL") in (None, ""),
        {"hits": test_in_train},
    )

    secret_hits = []
    for rel in git("ls-files").stdout.splitlines():
        if rel.startswith("scripts/p6/") or rel.startswith("docs/run-cards/p6/"):
            path = ROOT / rel
    ...
    env_text = (ROOT / "scripts/p6/env.sh").read_text(encoding="utf-8")
    record(
    ...
    payload = {
        ...
        "blinded": True,
        "p6_run_id": P6_RUN_ID,
        "script": "scripts/p6/gate_c_hygiene.py",
        "checks": checks,
        "test_access_count": 0,
        "p6_outcome_access_count": 0,
