#!/usr/bin/env python3
"""P6 parent-SHA reject list. Confirmatory P6 MUST NOT load prior-study weights."""
...
from p6_common import FORBIDDEN_P4_PARENT_SHA256  # noqa: E402
...
def reject_parent_sha256(digest: str) -> None:
    ...
    if d in FORBIDDEN_PARENT_SHA256:
        raise SystemExit(f"P6 forbids prior-study parent SHA256 {d}")


def reject_parent_path(path: str | Path) -> None:
    text = str(path).replace("\\", "/")
    if any(m in text for m in _FORBIDDEN_PATH_MARKERS):
        raise SystemExit(f"P6 forbids parent path from a prior study cache: {path}")
    p = Path(path)
