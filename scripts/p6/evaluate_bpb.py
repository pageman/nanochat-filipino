...
def load_gate_i_ckpt_sha(seed: int, depth: int) -> str:
    path = seed_card(seed) / f"gate-i-tl0-d{depth}.json"
    if not path.is_file():
...
def main() -> int:
    ...
    # Filed: untrained same-seed floor (Gate plan step 1).
    untrained_seed = seed
    tl0_depths = {8: tl0_tag(seed, 8), 20: tl0_tag(seed, 20)}

    os.environ.setdefault("NANOCHAT_BASE_DIR", str(ROOT / "data" / "cache" / P6_RUN_ID))
    ...
    # Scalar stays in lockbox log only; do not echo numeric BPB to operator chat.
    print("unigram floor computed", flush=True)
    ...
    results = {
        ...
        "tl_train_jsonl_sha256": EXPECTED["tl_train_jsonl"] if has_jsonl else None,
        "tl_val_jsonl_sha256": EXPECTED["tl_val_jsonl"] if has_jsonl else None,
        "packing": PACKING,
        "T": T,
    ...
    for depth, tag in tl0_depths.items():
        expected_ckpt_sha = load_gate_i_ckpt_sha(seed, depth)
