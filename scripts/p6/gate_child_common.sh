#!/usr/bin/env bash
# Shared P6 child train. Args: SEED GATE ARM TAG STREAM AUTH DATA_DIR
p6_child_train() {
  local SEED="$1" GATE="$2" ARM="$3" TAG="$4" STREAM="$5" AUTH="$6" DATA_DIR="$7"
  local gate_l
  gate_l="$(echo "$GATE" | tr '[:upper:]' '[:lower:]')"
  local LOCK_LOG="${P6_LOCKBOX_ROOT}/seed-${SEED}/gate-${gate_l}-${ARM}-full.log"
  local SAFE="${P6_SAFE_PROGRESS_ROOT}/seed-${SEED}/gate-${gate_l}-${ARM}-progress.txt"
  local FROZEN="${NANOCHAT_BASE_DIR}/p6-s${SEED}/c0/frozen/p6-s${SEED}-c0-tl-d20"
...
row = json.loads((root / "docs/run-cards/p6" / os.environ["P6_RUN_ID"] / f"seed-{seed}" / "gate-q-c0-freeze.json").read_text())
...
PY
  ...
  python scripts/p6/gate_phase2_accept.py \
