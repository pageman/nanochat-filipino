#!/usr/bin/env python3
"""C3-only dummy test gate. Rejects C1/C2 and missing U seal. Does not print test BPB."""
...
def main() -> int:
    ...
    if args.arm != "C3":
        print("test evaluator rejects non-C3 arm", file=sys.stderr)
        return 2
    if not (lockbox / "p4-validation-seal.json").exists():
        print("test evaluator rejects missing U seal", file=sys.stderr)
        return 2
    seal = json.loads((lockbox / "p4-validation-seal.json").read_text())
    ...
    (Path(args.safe_progress) / "gate-v-status.json").write_text(
        json.dumps({"one_authorized_C3_only_test_event_completed": True, "test_access_count": 1}) + "\n"
    )
    print("one authorized C3-only test event completed")
    return 0
