#!/usr/bin/env bash
# P5 CUDA-host environment. Source from the repo root on the named NVIDIA host.
# MUST NOT source scripts/p1/p2/p3/p4 env.sh

if [[ -n "${P4_ENV_SOURCED:-}" || -n "${P3_ENV_SOURCED:-}" ]]; then
  echo "P5 CUDA env refuses: prior-study env appears sourced" >&2
...
source "$NANOCHAT_FILIPINO_ROOT/scripts/p5/env.sh" || return 1 2>/dev/null || exit 1
...
echo "NOTE: P5 CUDA env only. Parent/children have not started. Confirmatory class is NVIDIA CUDA (A40 class)."
