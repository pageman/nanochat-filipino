...
def eval_model(model, batches, token_bytes, pack_meta: dict, label: str) -> dict:
    ...
    if os.environ.get("P5_EVAL_QUIET") != "1":
        print(
...
def load_gate_i_ckpt_sha(seed: int, depth: int) -> str:
    path = RUN_CARD / f"seed-{seed}" / f"gate-i-tl0-d{depth}.json"
    if not path.is_file():
        raise SystemExit(f"missing Gate I receipt: {path}")
    row = json.loads(path.read_text(encoding="utf-8"))
    if row.get("status") != "pass":
        raise SystemExit(f"Gate I_s d{depth} not pass")
...
def main() -> int:
    ...
    if args.seed not in (1, 2, 3):
        raise SystemExit("P5 P0-T seed must be 1, 2, or 3 (seed-matched floors)")
    tl0_depths = {8: f"p5-s{args.seed}-tl0-d8", 20: f"p5-s{args.seed}-tl0-d20"}
    untrained_seed = args.seed

    os.environ.setdefault("NANOCHAT_BASE_DIR", str(ROOT / "data" / "cache" / P5_RUN_ID))
    ...
    for depth, tag in tl0_depths.items():
        expected_ckpt_sha = load_gate_i_ckpt_sha(args.seed, depth)
        ...
        print(f"=== {tag} untrained seed={untrained_seed} (seed-matched) ===", flush=True)
