#!/usr/bin/env bash
# P5 Gate P0-T_s: seed-matched untrained floors + shared Tagalog byte-unigram.
set -euo pipefail
...
auth = Path(os.environ["P5_RUN_CARD_ROOT"]) / "seed-$SEED" / "gate-p0t-authorization.json"
...
BOX="${P5_LOCKBOX_ROOT}/seed-${SEED}"
SAFE="${P5_SAFE_PROGRESS_ROOT}/seed-${SEED}"
mkdir -p "$BOX" "$SAFE"
echo "gate_p0t_started seed=${SEED} utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee -a "$SAFE/gate-p0t-runner.txt"
set +e
python scripts/p5/evaluate_bpb.py \
...
python scripts/p5/gate_p0t_accept.py --seed "$SEED" --eval-exit-code "$RC"
exit $?
