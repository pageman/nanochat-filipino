#!/usr/bin/env bash
# P5 Gate H: Tagalog-path d4 CUDA smoke (NOT TL0 / NOT C0 / NOT per-seed parent).
set -euo pipefail
...
AUTH="$P5_RUN_CARD_ROOT/gate-h-authorization.json"
if [[ ! -f "$AUTH" ]]; then
...
import os
auth = Path(os.environ["P5_RUN_CARD_ROOT"]) / "gate-h-authorization.json"
...
LOCK_LOG="${P5_LOCKBOX_ROOT}/gate-h-smoke-full.log"
SAFE="${P5_SAFE_PROGRESS_ROOT}/gate-h-smoke-progress.txt"
mkdir -p "$(dirname "$LOCK_LOG")" "$P5_SAFE_PROGRESS_ROOT"
: >"$SAFE"
...
cd "$ROOT"
python scripts/p5/gate_h_accept.py --exit-code "$RC"
exit $?
