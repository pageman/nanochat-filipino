...
def main() -> int:
    ...
    next_gate = f"Q_{seed}" if status == "PASS" else (f"I_{seed + 1}" if status == "BLOCKED" and seed < 3 else "X")
    if status == "BLOCKED":
        write_json(
            seed_card(seed) / "ineligible_parent.json",
            {"seed": seed, "reason": "P0-T BLOCKED", "replacement_forbidden": True, "at_utc": utc_now()},
        )
    write_json(
        out,
        {
