#!/usr/bin/env bash
# P5 Gate I_s: fresh Tagalog TL0 (d8 then d20) with parent-init seed s.
set -euo pipefail
...
DEPTH="${2:-all}"
if [[ ! "$SEED" =~ ^[123]$ ]]; then echo "SEED must be 1, 2, or 3" >&2; exit 2; fi
if [[ "$(uname -s)" == "Darwin" ]]; then echo "REFUSE: Gate I is not Mac MPS." >&2; exit 3; fi
python scripts/p5/gate_i_preflight.py "$SEED"
python - <<PY
import json, os, sys
from pathlib import Path
auth = Path(os.environ["P5_RUN_CARD_ROOT"]) / "seed-$SEED" / "gate-i-authorization.json"
...
train_depth() {
  local depth="$1"
  local tag="p5-s${SEED}-tl0-d${depth}"
  local lock_log="${P5_LOCKBOX_ROOT}/seed-${SEED}/gate-i-tl0-d${depth}-full.log"
  local safe="${P5_SAFE_PROGRESS_ROOT}/seed-${SEED}/gate-i-tl0-d${depth}-progress.txt"
  ...
  set +e
  python "$NANOCHAT_FILIPINO_ROOT/scripts/p5/parent_train.py" \
    --parent-init-seed "$SEED" --wrapper-depth "$depth" \
    --record-initial-state "${P5_LOCKBOX_ROOT}/seed-${SEED}/initial_state_d${depth}.json" -- \
  ...
  cd "$ROOT"
  python scripts/p5/gate_i_accept.py --seed "$SEED" --depth "$depth" --exit-code "$rc"
  return $?
}
