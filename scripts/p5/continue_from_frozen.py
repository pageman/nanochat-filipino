#!/usr/bin/env python3
"""P5 continuation from frozen C0_s: parent weights only, fresh optimizer."""
...
from p5_common import N_PHASE2, N_TL0, WARMUP, child_tag, sha256_file  # noqa: E402
...
def validate(wrapper: argparse.Namespace, rest: list[str]) -> Path:
    ...
    expected_c0 = f"p5-s{wrapper.seed}-c0-tl-d20"
    if "c0" not in parts or "frozen" not in parts or expected_c0 not in parts:
        raise SystemExit(f"init-from must be frozen C0_s directory for seed {wrapper.seed}, got {init_dir}")
    ...
    if _has_flag(rest, "--resume-from-step"):
        ...
        if resume not in (None, "-1"):
            raise SystemExit("--resume-from-step loads optimizer; forbidden for P5 children")
    ...
    allowed = {child_tag(wrapper.seed, a) for a in ("c1", "c2", "c3")}
    ...
    frozen_root = (base / f"p5-s{wrapper.seed}" / "c0" / "frozen").resolve()
...
def install_weight_loader(init_dir: Path, init_step: int, expected_sha: str) -> None:
    ...
    def init_then_load_frozen(self):
        ...
        print(
            f"P5 continue_from_frozen: loaded C0 step {init_step} sha256={parent_sha} optimizer=fresh",
            flush=True,
        )
...
def main() -> None:
    ...
    print(f"P5 continue_from_frozen: parent_ok {model_path}", flush=True)
