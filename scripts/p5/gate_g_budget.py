...
BUDGET = ROOT / "manifests" / "p5" / "p5_budget_manifest.json"
DEPTHS = (8, 20)
ASPECT_RATIO = 64
HEAD_DIM = 128
WINDOW_PATTERN = "SSSL"
...
def main() -> int:
    ...
    parent_argv = {
        ...
        "sample_every": -1,
        "save_every": -1,
        "model_tags": ["p5-s{seed}-tl0-d8", "p5-s{seed}-tl0-d20"],
        "runs": ["p5-s{seed}-tl0-d8", "p5-s{seed}-tl0-d20"],
        "parent_init_seeds": [1, 2, 3],
        "note": "terminal checkpoint only; save-every=-1 writes model_{N:06d}.pt at last step on pin 92d63d4",
    }
    child_argv = {
        "wrapper": "python scripts/p5/continue_from_frozen.py",
        "init_from": f"$NANOCHAT_BASE_DIR/p5-s{{seed}}/c0/frozen/p5-s{{seed}}-c0-tl-d20",
        "init_step": N_TL0,
        "depth": 20,
        "max_seq_len": T,
        "window_pattern": WINDOW_PATTERN,
        ...
        "allowed_model_tags": ["p5-s{seed}-c1-tl-d20", "p5-s{seed}-c2-en-d20", "p5-s{seed}-c3-mix-d20"],
    }
    ...
    payload = {
        ...
        "blinded": True,
        "p5_run_id": P5_RUN_ID,
        "script": "scripts/p5/gate_g_budget.py",
        ...
        "cost_note": {
            ...
            "expected_gpus": "H + 3 x (I(d8) + I(d20) + R + S + T + U + V) for eligible seeds",
            "panel_seeds": [1, 2, 3],
