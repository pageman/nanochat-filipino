...
CKPT = BASE / "base_checkpoints" / "p5-smoke-tl-d4" / "model_000030.pt"
QUAR = LOCKBOX / "quarantine" / "p5-smoke-tl-d4"
...
def quarantine(src: Path) -> dict:
    ...
    smoke_dir = BASE / "base_checkpoints" / "p5-smoke-tl-d4"
    moved = []
    if smoke_dir.is_dir():
    ...
    in_c0 = list((BASE / "p5-s1").glob("**/*.pt")) if (BASE / "p5-s1").is_dir() else []
...
def main() -> int:
    ...
    payload = {
        ...
        "blinded": True,
        "p5_run_id": P5_RUN_ID,
        "script": "scripts/p5/gate_h_smoke.sh",
        "model_tag": "p5-smoke-tl-d4",
        "smoke_seed": 0,
        "depth": 4,
        "num_iterations": 30,
        "warmup_steps": 3,
        "eval_every": -1,
        "sample_every": -1,
        "nanochat_data_dir": f"data/cache/{P5_RUN_ID}/streams/c1_tl",
        "finite": finite,
