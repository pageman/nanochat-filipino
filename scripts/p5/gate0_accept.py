#!/usr/bin/env python3
"""P5 Gate 0 lockbox acceptance tests (dummy data only; no real val/test text)."""
...
def main() -> int:
    ...
    os.environ["P5_RUN_ID"] = P5_RUN_ID
    os.environ["NANOCHAT_BASE_DIR"] = str(CACHE)
    released = CACHE / "released_dummy"
    seed_lockbox_dirs()
    for d in (released,):
        d.mkdir(parents=True, exist_ok=True)
    PASSFILE.write_text("p5-gate0-dummy-passphrase-not-for-production-or-researchbox\n")
    ...
    tests["1"] = {
        "name": "filing/lock/config/evaluator + panel lockbox dirs",
        "pass": t1,
    ...
    for k in ("P4_TEST_JSONL_EN", "P4_TEST_JSONL_TL", "P5_TEST_JSONL_EN", "P5_TEST_JSONL_TL"):
        env.pop(k, None)
    sourced = subprocess.run(
        [
            "bash",
            "-lc",
            f"source {ROOT}/scripts/p5/env.sh && echo TEST_EN=${{P5_TEST_JSONL_EN-UNSET}} && echo TEST_TL=${{P5_TEST_JSONL_TL-UNSET}}",
        ],
        capture_output=True,
        text=True,
        env=env,
    ...
    try:
        ...
        t6 = True
    tests["6"] = {"name": "P1.1/P2/P3/P4 weight SHA256s rejected as parent", "pass": t6}
    ...
    r16 = subprocess.run(
        ["bash", "-lc", f"source {ROOT}/scripts/p4/env.sh && source {ROOT}/scripts/p5/env.sh"],
        capture_output=True,
        text=True,
        env={"NANOCHAT_FILIPINO_ROOT": str(ROOT), "P5_RUN_ID": P5_RUN_ID},
    )
    t16 = r16.returncode != 0 and "P5 env refuses" in (r16.stderr + r16.stdout)
    ...
    report = {
        ...
        "all_pass": all_pass,
        "tests": tests,
        "scripts_p5_sha256": scripts_sha,
        "scripts_p5_tree_sha256": tree_digest(scripts_sha),
        "no_p5_outcomes": True,
        "no_real_val_test_text": True,
        "researchbox_id": RESEARCHBOX_ID,
        "aspredicted_id": 307836,
        "note": "Dummy lockbox under gitignored data/cache. Panel seed subdirs present.",
    }
